...
class MediaFile(Storage):
    def __init__(self, download_path, file_name, title):
        ...
        self.has_video_extension = self.file_name.endswith(('mkv','avi','mp4','mov'))
    ...
    def move_media(self):
        ...
        if isinstance(self, MovieFile):
            ...
            Logger.log('[-] %s Season %s Episode %s wasn\'t found. Moving...' % self.title, self.season, self.episode)
        shutil.move('%s/%s' % (self.download_path, self.file_name), self.movie_root_path)

    def extract_media(self):
        from . import TvFile, MovieFile

        rar_files = glob.glob('*.rar')
        unrar_command = '%s x %s &>/dev/null' % (config.unrar_path, rar_files[0])
        try:
            destination_path = ''
            message = '[-] %s wasn\'t found. Extracting and moving...'
            if isinstance(self, TvFile):
                destination_path = self.tv_season_path
                Logger.log(message % '%s Season %s Episode %s' % (self.title, self.season, self.episode))
            ...
            extracted_files = flatten_list([glob.glob(extension) for extension in ['*.mkv','*.avi','*.mp4','*.mov']])
