...
class TvFile(MediaFile):
    ...
    def prepare_destination(self):
        self.tv_root_path = config.share_tv_root_path % (self.share_path, self.title)
        self.tv_season_path = config.share_tv_season_path % (self.share_path, self.title, self.season)

        if os.path.isdir(self.tv_root_path):
            if self.capacity_reached():
                Logger.log('[!] Capacity reached. Skipping adding Season %s of %s.' % (self.season, self.title))
            else:
                if not os.path.isdir(self.tv_season_path):
                    Logger.log('[+] Creating folder for Season %s of %s' % (self.season, self.title))
                    os.mkdir(self.tv_season_path)
        else:
            if self.capacity_reached():
                Logger.log('[!] Capacity reached. Skipping adding show %s.' % self.title)
            else:
